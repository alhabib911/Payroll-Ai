...
import { DollarSign, Users, Briefcase, TrendingUp, Download, Zap, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ employees, records, onProcessPayroll }) => {
  const totalPayroll = records.reduce((acc, curr) => acc + curr.grossSalary, 0);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs font-medium text-slate-400">Real-time enterprise payroll analytics and expenditure.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-lg border border-[#f0f0f0] shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-tight">Payroll Flow</h3>
        ...
        <div className="bg-white p-6 rounded-lg border border-[#f0f0f0] shadow-sm">
          <h3 className="text-sm font-bold text-slate-800 uppercase tracking-tight mb-6">By Department</h3>
          <div className="space-y-5">
            {deptData.length > 0 ? deptData.map((dept) => (
              <div key={dept.label}>
                <div className="flex justify-between text-[11px] font-bold mb-2">
                  <span className="text-slate-500">{dept.label}</span>
                  <span className="text-slate-800">{dept.value}%</span>
                </div>
